...
let user = parsedData.results;
let i = 0;

function userPicture() {
    let pictureURL = user[i].picture.large;
    return pictureURL;
}
...
function userName() {
    let userName = user[i].name.title;
    userName += " " + user[i].name.first;
    userName += " " + user[i].name.last;
    return userName;
}
...
function userInfo() {
    ...
    document.querySelector(".mobileText").innerHTML = email;
    document.querySelector(".phoneText").innerText = phone;
    picturePlace = document.querySelector("img");
    picturePlace.setAttribute("src", picture);
}
